...
<Information 
  title="Informasi Penting" 
  content="Untuk sementara, gambar contoh setiap perintah dapat Anda lihat pada file PDF modul."
  footer="Gambar contoh akan segera kami masukkan ke dalam website ini juga. Terima kasih! 🙏"
/>
...
4. **Install Flutter via FVM**
   ...
   <CodeBlock desc="Add FVM to PATH" language="bash" code={`
...
5. **Install Android SDK (tanpa Android Studio)**
   ...
   <CodeBlock desc="Check Architecture" language="bash" code={`
   arch
   `} />

   Jika hasilnya `arm64`, MacBook kamu menggunakan chip Apple Silicon (M1 ke atas).
   Jika hasilnya `i386` atau `x86_64`, MacBook kamu menggunakan chip Intel.
   ...
   **MacBook dengan Chip M1, M2, M3 (Apple Silicon)**
   ...
   <CodeBlock desc="Set PATH Android SDK (Apple Silicon)" language="bash" code={`
   ...
   **MacBook dengan Chip Intel**
   ...
   <CodeBlock desc="Set PATH Android SDK (Intel)" language="bash" code={`
   ...
   <CodeBlock desc="Reload ZSH Config" language="bash" code={`
   source ~/.zshrc
   `} />

   Setelah itu, jalankan script berikut ini:

   <CodeBlock desc="Install Android SDK Components" language="bash" code={`
