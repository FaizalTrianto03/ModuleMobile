...
***`1`. MediaQuery ( Mengetahui Kondisi Layar Global )***
...
***`2`. LayoutBuilder ( Menentukan Layout Berdasarkan Ruang Lokal )***
...
<CodeImport src="contoh7.dart" />

**Penjelasan Mekanisme**

`1`. LayoutBuilder menerima parameter constraints yang memberi tahu seberapa besar ruang tersedia untuk widget tersebut.

`2`. Grid menyesuaikan jumlah kolom berdasarkan maxWidth dari parent container.

`3`. Pada LayoutBuilderDemoPage, satu grid ditempatkan di container sempit (`250`px) dan satu lagi di area utama. Hasilnya: jumlah kolom di kedua grid berbeda, walau menggunakan widget yang sama.
...
<CodeImport src="contoh8.dart" />

**Penjelasan Mekanisme**

`1`. Slider mengubah nilai _itemSpacing, yang memengaruhi padding, crossAxisSpacing, dan mainAxisSpacing dari grid. Setiap perubahan nilai men-trigger setState(), memanggil ulang metode build(), lalu UI menyesuaikan jarak antar-item.

`2`. ElevatedButton digunakan untuk menguji perubahan ukuran elemen. Saat tombol ditekan, _scaleFactor berganti antara `1.0` dan `1.2`, lalu diterapkan pada childAspectRatio. Hasilnya, kartu produk akan tampak melebar atau memanjang secara proporsional.

`3`. Seluruh proses perubahan berjalan secara real-time, tanpa perlu restart atau reload manual, menunjukkan mekanisme stateful rebuild pada Flutter.
...
<CodeImport src="struktur10.txt" />

Deklarasikan hanya folder dasar di pubspec.yaml:

<CodeImport src="struktur11.txt" />
...
- Project di GitHub berisi branch untuk setiap modul (modul`1`, modul`2`, dan seterusnya).
...
`1`) Membangun UI adaptif menggunakan pendekatan MediaQuery dan LayoutBuilder.

`2`) Menerapkan animasi implisit sederhana menggunakan AnimatedContainer.

Buat sebuah aplikasi Flutter sederhana bertema Katalog Produk Dinamis, yang menampilkan antarmuka responsif dan animasi sederhana.

## ***Instruksi***

`1`) **Bangun satu halaman utama (misal: ProductCatalogPage).**
...
`2`) **Tambahkan efek animasi sederhana.**
...
`3`) **Uji Responsivitas dan Animasi.**
...
`1`. **Tidak perlu membuat navigasi, data API, atau aset eksternal.**

`2`. **Fokus pada dua kemampuan utama: adaptivitas tata letak dan transisi visual animasi.**

`3`. **Gunakan warna kontras agar perubahan tampilan mudah diamati.**
...
|---------------------|---------------------|
| **Codelab** |  |
| **Codelab `1`: Implementasi UI adaptif dengan MediaQuery dan LayoutBuilder** | `20`% |
| **Codelab `2`: Implementasi animasi sederhana dengan AnimatedContainer** | `20`% |
| **Demo** |  |
