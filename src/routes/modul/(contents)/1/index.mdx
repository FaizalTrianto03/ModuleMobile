...
1. **initState()** → dipanggil sekali ketika widget pertama kali dibuat. Cocok untuk inisialisasi data.
2. **build()** → dipanggil setiap kali UI harus digambar ulang.
3. **setState()** → memberi tahu Flutter bahwa ada perubahan state yang memengaruhi tampilan.
4. **dispose()** → dipanggil ketika widget dihapus dari tree, biasanya digunakan untuk membersihkan resource.
...
<Information 
  ...
  content="Submit project dalam bentuk ZIP file atau link GitHub repository. Pastikan README.md sudah lengkap dengan penjelasan dan screenshot."
  type="warning"
/>
