---
title: "Modul 3: API Call & Async Handling"
description: "Implementasi pemanggilan API (API Call) dan pengelolaan proses asinkron (Async Handling) menggunakan library http dan Dio di Flutter."
---

import { MockupWindowCode } from "/src/components/MockupWindowCode";
import { Information } from "~/components/modul-list";
import { CodeBlock } from "/src/components/CodeBlock";

/* Catatan perbaikan build:
   - Jangan gunakan komentar HTML (<!-- ... -->) atau C-style (/* ... * /) di dalam konten MDX. Gunakan komentar JSX: {/** ... * /}.
   - Hindari karakter spesial seperti ! di awal baris CSS/JS (contoh: /*! ... */). Jika harus ditampilkan, bungkus dalam string atau komponen.
   - Hindari menyisipkan { /* ... */ } di dalam komentar blok ini; gunakan satu gaya komentar saja.
*/

/* Jika file-file berikut SUDAH tersedia, Anda bisa aktifkan dua import di bawah:
   import contoh1Code from "./code/apipublik.sh?raw";          // URL/API publik
   import contoh2Code from "./code/response.txt?raw";          // Contoh response JSON
*/
import contoh3Code from "./code/httppubspec.txt?raw";       // pubspec: http
import contoh4Code from "./code/contohhttp.dart?raw";       // fetchCountries (http)
import contoh5Code from "./code/contohhttpui.dart?raw";     // FutureBuilder UI
import contoh6Code from "./code/diopubspec.txt?raw";        // pubspec: Dio
import contoh7Code from "./code/contohdio.dart?raw";        // fetchCountries (Dio)
import contoh8Code from "./code/contohfuture.dart?raw";     // Future sederhana
import contoh9Code from "./code/asyncawait.dart?raw";       // async-await dasar
import contoh10Code from "./code/callbackchaining.dart?raw";// callback chaining
import contoh11Code from "./code/contohlogin.dart?raw";     // Future boolean login()
import contoh12Code from "./code/asyncawaitlogin.dart?raw"; // login async-await
import contoh13Code from "./code/callbacklogin.dart?raw";   // login callback

<Information
  title="Informasi Penting"
  content="<strong>Modul Mobile dalam versi website ini dibuat khusus untuk memudahkan copy-paste code dan command yang ada.</strong><br/>Untuk materi lengkap dan penjelasan detail, silakan unduh dan baca <strong>file PDF modul</strong> yang tersedia."
  footer="Terima kasih atas perhatiannya! 🙏"
/>

<Information
  title="Download PDF Modul"
  content="Silakan download file PDF modul lengkap untuk penjelasan detail."
  type="info"
  link={{
    text: "Download PDF Modul",
    url: "https://drive.google.com/drive/folders/1gN3M1QrANcIi7qtuiCTI_Ge_UPAV67Xc?usp=sharing",
    external: true
  }}
/>

<Information
  title="Pengisian Data Kelompok"
  content="Silakan isi nama anggota kelompok pada link berikut ini. Data ini akan digunakan untuk Modul 2 hingga Modul 6, jadi pastikan semua anggota sudah tercantum dengan benar."
  type="warning"
  link={{
    text: "Isi Sheet Nama Kelompok",
    url: "https://docs.google.com/spreadsheets/d/1mbAsIngJ51xHSH3t59S4JriNnkGKTQKWB0H4wOnQ_3E/edit?usp=sharing",
    external: true
  }}
/>

VERSI 1.0  
OKTOBER 2025

# PEMROGRAMAN MOBILE

## MODUL 3 - Materi

## API Call & Async Handling

DISUSUN OLEH:  
Ali Sofyan Kholimi, M.Kom.  
Faizal Qadri Trianto  
Muhammad Hisyam Kamil

<a id="pendahuluan"></a>
# PENDAHULUAN

<a id="tujuan"></a>
## TUJUAN

1. Mengimplementasikan pemanggilan API (API Call) dan pengelolaan proses asinkron (Async Handling).
2. Menilai performa library HTTP serta kemudahan proses debugging pada operasi asinkron.

<a id="target-modul"></a>
## TARGET MODUL

1. Mahasiswa mampu membuat aplikasi sederhana yang memanfaatkan API menggunakan library http dan Dio.
2. Mahasiswa mampu membandingkan implementasi proses asinkron dengan pendekatan async-await dan callback chaining.

<a id="persiapan"></a>
## PERSIAPAN

1. Menggunakan API publik, seperti REST Countries atau OpenWeatherMap.
2. Menyiapkan Stopwatch atau DevTools Profiler untuk pencatatan waktu (logging) dan pelacakan kesalahan (error trace).

<a id="keywords"></a>
## KEYWORDS

REST API, http, Dio, async-await, callback

<a id="table-of-contents"></a>
## TABLE OF CONTENTS

[ PENDAHULUAN ](#pendahuluan)

[TUJUAN](#tujuan)

[TARGET MODUL](#target-modul)

[PERSIAPAN](#persiapan)

[KEYWORDS](#keywords)

[TABLE OF CONTENTS](#table-of-contents)

[ API CALL & ASYNC HANDLING ](#api-call-async-handling)

[Pentingnya Desain Responsif pada Mobile](#pentingnya-desain-responsif-pada-mobile)

[Implementasi API Call dengan http](#implementasi-api-call-dengan-http)

[Implementasi API Call dengan Dio](#implementasi-api-call-dengan-dio)

[Dasar Asynchronous Programming](#dasar-asynchronous-programming)

[Simulasi Login (Async)](#simulasi-login-async)

[Rekap Materi](#rekap-materi)

[ AKSES PROJECT & REFERENSI MODUL ](#akses-project-referensi)

[ CODELAB ](#codelab)

[Tugas](#tugas)

[Instruksi](#instruksi)

[Catatan](#catatan)

[ KRITERIA & DETAIL PENILAIAN ](#kriteria-detail-penilaian)

---

<a id="api-call-async-handling"></a>
# API CALL & ASYNC HANDLING

<a id="pentingnya-desain-responsif-pada-mobile"></a>
## Pentingnya Desain Responsif pada Mobile

Dalam pengembangan aplikasi modern, sebagian besar data tidak disimpan secara lokal, melainkan diambil melalui API (Application Programming Interface). API adalah jembatan yang memungkinkan aplikasi client berinteraksi dengan server untuk mengambil, mengirim, atau memperbarui data.

1. Siklus Request–Response API

Tiga tahap utama:
1) Client mengirimkan request — menggunakan metode HTTP seperti GET, POST, PUT, DELETE.  
2) Server memproses permintaan — menyiapkan hasil (umumnya JSON).  
3) Client menerima response — melakukan decode, lalu menampilkan ke UI.  

Contoh API publik (REST Countries):

{/** Inline literal agar build aman jika file belum ada */}
<CodeBlock desc="URL API Publik" language="bash" code={`curl -s https://restcountries.com/v3.1/all?fields=name`} />

Contoh hasil response:

{/** Inline literal agar build aman jika file belum ada */}
<CodeBlock desc="Contoh Response JSON" language="json" code={`[
  { "name": { "common": "Indonesia", "official": "Republic of Indonesia" } },
  { "name": { "common": "Japan", "official": "Japan" } }
]`} />

Setiap objek pada array berisi informasi nama negara (common/official) yang dapat kita tampilkan.

2. REST API dan Konsep Data JSON

- REST memanfaatkan HTTP standar; JSON dipilih karena ringan dan mudah dibaca.  
- Ciri utama: endpoint data, metode HTTP, response JSON.  
- Data JSON harus di-decode (dart:convert) agar dapat diakses sebagai map/objek di Dart/Flutter.

3. Perbandingan REST API dan GraphQL (sekilas)

- REST: endpoint tetap, sederhana, umum digunakan.  
- GraphQL: query fleksibel, hanya data yang diminta, lebih kompleks (butuh server khusus).  

Modul ini menggunakan REST API karena sederhana dan umum di Flutter.

<a id="implementasi-api-call-dengan-http"></a>
## Implementasi API Call dengan http

Langkah 1: Tambahkan dependency

<CodeBlock desc="Menambahkan dependency http" language="plaintext" code={contoh3Code} />

Langkah 2: Buat fungsi pengambil data

<CodeBlock desc="Fungsi fetchCountries dengan http" language="dart" code={contoh4Code} />

Langkah 3: Tampilkan hasil pada UI

<CodeBlock desc="Tampilkan data dengan FutureBuilder" language="dart" code={contoh5Code} />

Penjelasan Mekanisme

- FutureBuilder menunggu Future (`fetchCountries()`) dan membangun ulang UI saat hasil tersedia.  
- Saat loading, tampilkan CircularProgressIndicator agar UI tetap responsif.  
- snapshot.hasError menangani kondisi error (mis. koneksi gagal) dengan menampilkan pesan yang informatif.  

Observasi

- Jaringan aktif: data tampil (Future sukses).  
- Jaringan lambat: loading tetap responsif karena operasi async non-blocking.  
- Error koneksi: tampil pesan error (handling bekerja).  

<a id="implementasi-api-call-dengan-dio"></a>
## Implementasi API Call dengan Dio

Dio menawarkan fitur lebih lengkap: interceptor, logging, timeout, cancel request, dsb.

Langkah 1: Tambahkan dependency

<CodeBlock desc="Menambahkan dependency Dio" language="plaintext" code={contoh6Code} />

Langkah 2: Buat fungsi pengambil data

<CodeBlock desc="Fungsi fetchCountries dengan Dio" language="dart" code={contoh7Code} />

Kelebihan Dio

- response.data langsung siap pakai (otomatis parsing JSON).  
- Logging request/response mudah via interceptor.  
- Error handling detail (timeout, status code, dsb).  

Perbandingan Singkat

- Parsing: http (jsonDecode manual) vs Dio (response.data otomatis).  
- Logging: http perlu solusi tambahan vs Dio built-in via interceptor.  
- Error handling: http dasar vs Dio lebih kaya.  
- Kompleksitas: http sederhana vs Dio fleksibel/powerful.  

<a id="dasar-asynchronous-programming"></a>
## Dasar Asynchronous Programming

1. Konsep Dasar Future

Future mewakili nilai yang tersedia di masa depan. Fungsi async langsung mengembalikan Future, proses internal tetap berjalan di "belakang layar".

<CodeBlock desc="Contoh Future sederhana" language="dart" code={contoh8Code} />

2. Event Loop: Jantung Asynchronous di Dart

- UI berjalan di main thread; tugas berat dimasukkan ke event queue.  
- Sambil menunggu hasil, UI tetap menggambar dan merespons input pengguna.  
- Saat Future selesai, event loop mengeksekusi callback yang terkait.  

3. async-await vs Callback

a. async–await (lebih mudah dibaca, linear)

<CodeBlock desc="Contoh async-await" language="dart" code={contoh9Code} />

b. callback chaining (.then/.catchError)

<CodeBlock desc="Contoh callback chaining" language="dart" code={contoh10Code} />

Perbandingan

- Keterbacaan: async-await lebih linear dan bersih.  
- Debugging: async-await lebih mudah dilacak.  
- Performa: setara.  
- Rekomendasi: gunakan async-await untuk alur menengah–kompleks.  

<a id="simulasi-login-async"></a>
## Simulasi Login (Async)

Gunakan Future.delayed 2 detik untuk meniru panggilan API autentikasi.

<CodeBlock desc="Fungsi simulasi login" language="dart" code={contoh11Code} />

1. Versi async–await

<CodeBlock desc="Login dengan async-await" language="dart" code={contoh12Code} />

2. Versi callback

<CodeBlock desc="Login dengan callback" language="dart" code={contoh13Code} />

Observasi

- async–await memberikan alur yang mudah diikuti dan di-debug.  
- Callback cocok untuk kasus sederhana; hindari nested callback pada alur kompleks.  

<a id="rekap-materi"></a>
## Rekap Materi

Pada bab ini Anda mempelajari:
1) Siklus request–response API dan pemanggilan data via http serta Dio.  
2) Cara kerja asynchronous di Dart (Future, event loop).  
3) Perbandingan async-await dengan callback chaining.  
4) Simulasi login yang meniru proses async di dunia nyata.  

Dengan bekal ini, Anda siap menerapkan API call dan async handling pada aplikasi Flutter.

---

<a id="akses-project-referensi"></a>
## AKSES PROJECT & REFERENSI MODUL

- Repository GitHub:  
  [GITHUB](https://github.com/FaizalTrianto03/Prak.Mobile-25-26)

- Website Modul (untuk memudahkan menyalin contoh kode):  
  [Website Modul](https://mobile.faizaltrianto.com/modul/3/)

Catatan:
- Repositori memiliki branch per modul (modul1, modul2, modul3, ...).  
- Website modul membantu membaca dan menyalin snippet kode secara langsung.  

---

<a id="codelab"></a>
# CODELAB

<a id="tugas"></a>
## Tugas

Jalankan dan amati aplikasi Flutter yang memanggil data dari REST Countries API menggunakan dua library berbeda: http dan Dio. Codelab ini juga dapat dipadukan dengan state management seperti GetX agar perubahan data otomatis memperbarui UI.

<a id="instruksi"></a>
## Instruksi

1) Clone Project
- Lakukan cloning repository berikut pada branch modul3:  
  [Project Github](https://github.com/FaizalTrianto03/Prak.Mobile-25-26)

2) Jalankan Project
- Buka project di VS Code atau Android Studio, kemudian:
  - flutter pub get
  - flutter run

3) Verifikasi API Call
- Endpoint yang digunakan:
  - https://restcountries.com/v3.1/all?fields=name
- Pastikan project menyediakan dua implementasi: via http dan via Dio.
- Pastikan data negara muncul pada UI, menampilkan setidaknya name.common dan name.official.

4) (Opsional) Gunakan GetX
- Jika menggunakan GetX untuk state management:
  - Buat controller untuk memuat data negara (observable list).
  - Ikat UI ke state GetX (Obx/GetBuilder) agar UI auto-rebuild saat data berubah.
  - Bandingkan kemudahan dibanding setState().

<a id="catatan"></a>
## Catatan

1. Fokus utama: project berjalan dan data negara termuat dari endpoint.  
2. Sediakan dua versi pemanggilan (http dan Dio) agar bisa dibandingkan.  
3. Jika menerapkan GetX, jaga struktur folder (controllers, views, services) rapi.  
4. Untuk profiling, gunakan DevTools Network/CPU untuk mengamati waktu respons dan beban pemrosesan.  

---

<a id="kriteria-detail-penilaian"></a>
# KRITERIA & DETAIL PENILAIAN

| Kriteria Penilaian | Persentase Penilaian |
| --- | --- |
| API call berhasil memuat data | 40% |
| Dokumentasi hasil profiling API | 20% |
| Analisis perbandingan readability asinkron | 20% |
| Argumentasi dan diskusi kasus penggunaan | 20% |
