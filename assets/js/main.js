...
function determinePageType(path) {
    if (path.includes('index.html') || path === '/' || path.endsWith('/learning-platform/')) {
        return 'home';
    } else if (path.includes('modules.html')) {
        return 'modules';
    } else if (path.includes('modul/')) {
        return 'module';
...
function isActiveNavItem(url) {
    const currentPath = window.location.pathname;
    return currentPath.includes(url) || (url === '/index.html' && (currentPath === '/' || currentPath.includes('index.html')));
}
...
async function initializeHomePage() {
    try {
        ...
        updateBreadcrumb(data.breadcrumb || [{ title: 'Home', url: '/index.html' }]);
    } catch (error) {
...
async function initializeModulesPage() {
    try {
        ...
        updateBreadcrumb([
            ...
            { title: 'Modules', url: '/modules.html' }
        ]);
    } catch (error) {
...
async function initializeStaticPage(pageType) {
    try {
        ...
        console.error(`Error loading ${pageType} page:`, error);
        ...
        <div class="bg-white dark:bg-gray-800 rounded-lg shadow-md hover:shadow-lg transition-shadow duration-300 overflow-hidden">
            ...
            <div class="p-6">
                ...
                <div class="flex items-center justify-between">
                    <a href="${module.url}" class="inline-flex items-center px-4 py-2 bg-primary-orange text-white rounded-lg hover:bg-opacity-90 transition-opacity">
...
function updateBreadcrumb(breadcrumbItems) {
    ...
    breadcrumb.innerHTML = breadcrumbItems.map((item, index) => {
        ...
        if (isLast) {
            ...
            return `
                <a href="${item.url}" class="text-primary-orange hover:text-accent-orange">${item.title}</a>
...
function initializeModuleNavigation(moduleData) {
    if (moduleData.navigation) {
        ...
        // Add navigation at the bottom of content
        const contentContainer = document.getElementById('page-content');
        const navHTML = `
            <div class="flex justify-between items-center mt-12 pt-8 border-t border-gray-200 dark:border-gray-700">
                ...
                ${next ? `
                    <a href="${next.url}" class="flex items-center px-6 py-3 bg-primary-orange text-white hover:bg-opacity-90 rounded-lg transition-opacity">
    ...
    if (contentContainer) {
        contentContainer.innerHTML = `
            <div class="max-w-2xl mx-auto text-center py-12">
                ...
                <button onclick="window.location.reload()" class="px-6 py-2 bg-primary-orange text-white rounded-lg hover:bg-opacity-90">
                    Try Again
                </button>
            </div>
        `;
    ...
    sidebar.innerHTML = `
        ...
        <nav class="flex-1 p-4">
            <ul class="space-y-2">
                ...
                <li><a href="/modules.html" class="flex items-center px-3 py-2 text-sm font-medium text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-700 rounded-lg">
    ...
    if (contentContainer) {
        contentContainer.innerHTML = `
            <div class="text-center py-12">
                <h1 class="text-4xl font-bold mb-4">Welcome to Mobile Development Platform</h1>
                <p class="text-lg text-gray-600 dark:text-gray-300 mb-8">Learn mobile app development step by step</p>
                <a href="/modules.html" class="inline-flex items-center px-6 py-3 bg-primary-orange text-white rounded-lg hover:bg-opacity-90">
...
function renderFallbackModulesPage() {
    ...
    if (contentContainer) {
        contentContainer.innerHTML = `
            <div class="text-center py-12">
                <h1 class="text-3xl font-bold mb-4">Learning Modules</h1>
...
function renderFallbackModulePage() {
    ...
    if (contentContainer) {
        contentContainer.innerHTML = `
            <div class="text-center py-12">
                <h1 class="text-3xl font-bold mb-4">Module Content</h1>
...
function renderFallbackStaticPage(pageType) {
    ...
    if (contentContainer) {
        contentContainer.innerHTML = `
            <div class="text-center py-12">
                <h1 class="text-3xl font-bold mb-4">${pageType.charAt(0).toUpperCase() + pageType.slice(1)}</h1>
...
// Initialize breadcrumb separately for better UX
